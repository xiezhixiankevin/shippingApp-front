<template>
    <div class="page-background">
      ...
      <h1 class="title-text">已发货运输信息</h1>
      <table class="transport-table">
        ...
        <tbody>
          <tr v-for="(item, index) in transportationList" :key="item.id">
            ...
            <td>{{ item.orderNum }}</td>
            <td>
              <button class="start-transport" @click="startTransportation(item.id, index)">设置到达</button>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  export default {
    ...
    methods: {
      async getTransportationWaitingInfo() {
        try {
          const response = await axios.get("/carrier/get-transportation-in-transit-info");
      ...
      async startTransportation(id, index) {
        try {
          const response = await axios.post("/carrier/end-transportation", { id });
